  ...
  <div>
    ...
    <el-table ref="dataTable" :data="tableData" border stripe style="width: 100%; margin-top: 20px" :highlight-current-row="true" @current-change="handleRowChange" :height="550">
      ...
      <el-table-column prop="status" label="状态" width="80">
        <template slot-scope="scope">
          <!-- 值得类型如果是int，那么active-value和inactive-value要加冒号 -->
          <el-switch
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible">
      <el-form ref="form" :model="dialogForm" :rules="rules" label-width="80px">
        <el-form-item label="字典编码" prop="dictCode">
          <el-input v-model="dialogForm.dictCode"></el-input>
        </el-form-item>
        ...
        <el-form-item label="状态">
          <!-- 这里label如果也是int值，同样需要加冒号 -->
          <el-radio v-model="dialogForm.status" :label="1">启用</el-radio>
          <el-radio v-model="dialogForm.status" :label="2">未启用</el-radio>
        </el-form-item>
      </el-form>
...
<script>
import {queryDictPage, queryDictDetail, addDict, updateDict, deleteDict} from "@/api/system";
...
export default {
  ...
  methods: {
    ...
    queryDetail(key) {
      ...
      return Object.assign([], this.currentRow.dictDetailList);
      /* TODO 异步请求，获取数据问题
      let data = [];
      queryDictDetail(key).then(res => {
        data = res.data.data;
    ...
    // 查询状态字典
    getStatus() {
      queryDictDetail("sys_status").then(res => {
    ...
    // 打开修改界面
    openUpdateDialog() {
      if (this.currentRow.sysFlag === 1) {
        this.$message.error("系统字典禁止编辑！");
        return
      }
      this.dialogTitle = '修改';
      this.dialogVisible = true;
    ...
    deleteDict() {
      if (this.currentRow.sysFlag === 1) {
        this.$message.error("系统默认字典禁止删除！");
        return
      }
      this.$confirm('确定删除字典<' + this.currentRow.dictName + '>吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteDict(this.currentRow.id).then(res => {
          if (res.data.status === 1) {
            this.$message({
              message: res.data.msg,
              type: 'success'
    ...
    openDetail() {
      ...
      this.detailList = Object.assign([], this.currentRow.dictDetailList);
    },
    // 子组edit-table件返回的时间
